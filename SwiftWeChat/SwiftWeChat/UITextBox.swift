...
class UITextBox: UITextField {
    ...
    var highlightState:UITextBoxHighlightState = .Default {
        didSet {
            ...
            UIView.animateWithDuration(TimeInterval(animateDuration)) {
    ...
    func layoutPlaceholderLabel(rect: CGRect,_ left: Bool = false) {
        ...
        UIView.animateWithDuration(TimeInterval(animateDuration), delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0, options: UIViewAnimationOptions.curveLinear, animations: {
            label.frame = frame;
        }, completion: nil)
    }
